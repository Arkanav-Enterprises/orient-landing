...
export default function PatronsPage() {
  return (
    <PageShell title="Our Patrons" subtitle="Imprinting new standards in printing and packaging machinery across the world.">
      <p className="text-[18px] font-medium text-white/50 leading-[1.5] mb-12" style={{ maxWidth: 700 }}>
      ...
      <div className="flex flex-wrap gap-3">
        {countries.map((c) => (
          <span key={c} className="text-[15px] font-medium text-white/40 px-4 py-2.5 border border-white/10 rounded-[4px]">{c}</span>
        ))}
      </div>
    </PageShell>
  );
