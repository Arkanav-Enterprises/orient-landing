  ...
  return (
    <PageShell title="Our Infrastructure" subtitle="A 20-acre manufacturing campus in Delhi NCR with three production units.">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-16">
        {plants.map((p) => (
          <div key={p.name} className="bg-black/[0.03] rounded-[6px] p-8">
            ...
            <h3 className="text-[20px] font-medium text-near-black mb-3">{p.name}</h3>
            <p className="text-[16px] font-medium text-near-black/40 leading-[1.5]">{p.desc}</p>
          </div>
        ))}
      </div>
      <p className="text-[18px] font-medium text-near-black/50 leading-[1.5]">
