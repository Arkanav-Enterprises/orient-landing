  ...
  return (
    <PageShell title="Our Team" subtitle="Three generations of leadership driving innovation across printing and solar energy.">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {leaders.map((l) => (
          <div key={l.name} className="bg-black/[0.03] rounded-[6px] overflow-hidden">
            <div className="bg-[#1a1a1a] flex items-center justify-center" style={{ height: 280 }}>
              <span className="text-near-black/10 text-[12px] font-medium">Photo</span>
            </div>
            <div className="p-6">
              <h3 className="text-[18px] font-medium text-near-black mb-1">{l.name}</h3>
              <p className="text-[15px] font-medium text-near-black/40">{l.title}</p>
            </div>
          </div>
        ))}
      </div>
      <p className="text-[18px] font-medium text-near-black/40 mt-12 leading-[1.5]">
